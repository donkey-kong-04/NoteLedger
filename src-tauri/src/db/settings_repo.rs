...
pub fn load(conn: &Connection) -> Result<UserSettings> {
    conn.query_row(
        "SELECT category1_label, category2_label, category3_label, category4_label, dark_mode, density
...
pub fn save(conn: &Connection, s: &UserSettings) -> Result<()> {
    conn.execute(
        "UPDATE user_settings SET
            ...
            dark_mode = ?5, density = ?6
         WHERE id = 1",
        params![s.category1_label, s.category2_label, s.category3_label, s.category4_label, s.dark_mode, s.density],
    )?;
    Ok(())
}
