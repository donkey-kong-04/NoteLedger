...
The app has a global **navigation bar** at the very top (shared by every page via the layout): a "Note Ledger" text wordmark sits at the left edge (bold, theme text color — dark in light mode, light in dark mode; no icon), followed by the **app version** in smaller muted text (e.g. "Note Ledger 0.4.5") — read at build time from `package.json` via a Vite JSON import in `+layout.svelte`, so it updates automatically with every version bump. A centered segmented tab control (`NavTabs.svelte`) switches between the views — **Project view | Table view | Templates | Settings**. The active tab is highlighted as a solid accent-colored pill with white text; the strip background is a darker surface shade for contrast. New pages are added by appending to the `tabs` array in `NavTabs.svelte`.
...
All filters live in a single component, `FilterPanel.svelte`, used by both the Project view and the Table view. It renders these things:
...
2. **Quick-search (magnifier) icon button**, right of the funnel. One click opens the drawer, focuses the **project lookup** input and opens its dropdown, so you can type a project name immediately. Like focusing the lookup manually, it clears the current project filter to start a fresh search.
3. **✕ Clear all button** (red outline), right of the icons, visible **only while at least one filter is active**. Clears every filter at once: project, log type, all category selections and "Show closed". (This used to live in the drawer header; it is now always reachable without opening the drawer.)
4. **Active-filter chips**, shown next to the buttons at all times (even with the drawer closed) so active filters are never hidden state. One chip per active filter — selected project, log type, each selected category value (tinted with its category color), and "Closed shown". Each chip has an **×** that clears just that filter.
5. **The drawer** itself: a non-modal panel (440px) that slides in from the left edge and **pushes the page content to the right** instead of overlaying it (the app shell gets matching left padding, animated in sync), so the main view stays fully visible and updates live as filters are toggled. Closed via the funnel icon, the ✕ button, **Escape**, or clicking anywhere outside the drawer. Open state lives in the `filterDrawerOpen` store so the layout can react; it resets when navigating to a page without filters. Header row: "Filters" title and the close button. Body sections:
...
- **Body** (shown when the project has logs or links): a **Logs | Links** tab bar in a folder-tab style. Tabs take only their **natural width** (label + count pill), sitting at the left of a full-width strip whose background is the **same `--project-header` color as the card's header bar**, so header + strip read as one piece. Each tab has both top corners rounded (7px). There are **no separator borders anywhere** — tabs are distinguished by background alone: the **active tab takes the card body's background color and merges seamlessly with the panel below**, like a physical folder tab, with bold text and a thin 2px accent line along its top edge; inactive tabs are simply strip-colored shapes in muted text. The global accent is deliberately NOT used as a fill here — solid accent pills are reserved for global chrome (nav bar, primary buttons, active filters). Each tab shows a count pill. **Logs** is the default selection. The tab choice is per-card and resets to Logs when the card is re-rendered (e.g. after folding). Sub-projects are NOT part of the tabs — they render below the card regardless of the selected tab.
