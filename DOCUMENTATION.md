...
- **Body** (shown when the project has logs or links): a full-width **Logs | Links** tab bar in a folder-tab style: the two tabs sit flush against each other (a 1px seam border between them), rounded only on the bar's outer top corners. The **active tab takes the card body's background color and merges seamlessly with the panel below** (its bottom border disappears), like a physical folder tab, with bold text and a thin 2px accent line along its top edge; inactive tabs sit recessed on a darker strip in muted text. The global accent is deliberately NOT used as a fill here — solid accent pills are reserved for global chrome (nav bar, primary buttons, active filters). Each tab shows a count pill. **Logs** is the default selection. The tab choice is per-card and resets to Logs when the card is re-rendered (e.g. after folding). Sub-projects are NOT part of the tabs — they render below the card regardless of the selected tab.
